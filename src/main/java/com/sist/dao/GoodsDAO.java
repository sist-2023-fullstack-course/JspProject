...
public class GoodsDAO {
	...
	public List<GoodsVO> goodsListData(int page, int type)
	{
		...
		try {
			...
			String sql="SELECT no, goods_poster, goods_price, goods_name, num "
					...
					+ "FROM "+table[type]+" ORDER BY no ASC)) "
	...
	public int goodsTotalPage(int type)
	{
		...
		try {
			conn=db.getConnection();
			String sql="SELECT CEIL(COUNT(*)/12.0) FROM "+table[type];
			ps=conn.prepareStatement(sql);
			ResultSet rs=ps.executeQuery();
	...
	public GoodsVO goodsDetailData(int no, int type)
	{
		...
		try {
			conn=db.getConnection();
			String sql="UPDATE "+table[type]+" SET "
					+ "hit=hit+1 "
					+ "WHERE no=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, no);
			ps.executeUpdate(); // commit() 포함 : autocommit() 자동커밋
			
			sql="SELECT no, goods_poster, goods_name, goods_sub, "
					+ "goods_price, goods_first_price, goods_discount, "
			...
			vo.setGoods_discount(rs.getShort(7));
			...
			// temp="19,900원" => 19900
			temp=temp.replaceAll("[^0-9]", ""); // 숫자를 제외하고 나머지는 공백으로 채워라
